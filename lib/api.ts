...
export interface UpdateClientPayload {
   ...
   * These are forwarded to the backend if provided.
   */
...
// Clients (business onboarding) — password is generated by backend if omitted
...
export async function resetClientPasswordApi(clientId: string) {
  return apiFetch<ClientCredentials>(`/clients/${clientId}/reset-password`, {
    method: "POST",
  });
}
...
export async function getLocationByIdApi(id: string) {
  return apiFetch<Location>(`/locations/${id}`);
}
...
export async function createFacilityForLocationApi(
  locationId: string,
...
export async function getFacilityByIdApi(locationId: string, facilityId: string) {
  return apiFetch<Facility>(
    `/locations/${locationId}/facilities/${facilityId}`
  );
}
